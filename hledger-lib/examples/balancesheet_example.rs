    ...
    match get_balancesheet(Some("tests/fixtures/test.journal"), &options) {
        Ok(report) => {
            ...
            for subreport in &report.subreports {
                println!(
                    "  {} (increases_total: {}): {} rows",
                    subreport.name,
                    subreport.increases_total,
                    subreport.rows.len()
                );
    ...
    match get_balancesheet(Some("tests/fixtures/test.journal"), &options) {
        Ok(report) => {
            ...
            for subreport in &report.subreports {
                println!(
                    "  {} (increases_total: {}):",
                    subreport.name, subreport.increases_total
                );
                for row in &subreport.rows {
                    println!("    {}", row.account);
                }
